...
class PluginGenerator_ActionAdminSamolet extends PluginGenerator_Inherits_ActionAdmin {
    /**
     * Регистрация евентов
     */
    protected function RegisterEvent() {
        parent::RegisterEvent();
        $this->AddEvent('tester', 'EventTest');
    }
	...
	protected function EventGeneratorTest() {
        $this->sMainMenuItem = 'content'; 
        $nPage = $this->_getPageNum();
		
		// $oGeneratorTest = Engine::GetEntity('PluginGenerator_ModuleGenerator_EntityTest');
		// $oGeneratorTest->setPlugin('ttt');
		// $oGeneratorTest->setEntity('ttt2');
		// $oGeneratorTest->Save();
		
		
        $aGeneratorTest = $this->PluginGenerator_Generator_GetTestItemsByFilter(
            array(
                '#page' => 1,
        ...
        $aPaging = $this->Viewer_MakePaging(
            $aGeneratorTest['count'], $nPage, Config::Get('admin.items_per_page'), 4,
            Router::GetPath('admin') . 'generator_test/'
        );
        $this->Viewer_Assign('aGeneratorTest', $aGeneratorTest['collection']);
        $this->Viewer_Assign('aPaging', $aPaging);
        $this->SetTemplateAction('generator_list');
    }
